...
public class FragmentOne extends ListFragment implements AdapterView.OnItemClickListener {
...

    {
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                ...
                flag_loading = true;
                URL url = new URL(C_SERVER_PROD + "asks.php");
        ...
        protected void onPostExecute(String result) {
            ...
            if (allitems.size() == 0) {
                ...
                for (int i = 0; i < allitems.size(); i++) {
                    ...
                    temp.put(C_ISMYID, allitems.get(i).get(6));
                    list.add(temp);
                }
    ...
    public class listviewProximityAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = inflater.inflate(R.layout.asks_layout, null);
            ...
            if (map.get(C_ISMYID).toString().equals(myid))
            {
            ...
            holder.Ibelieve.setOnClickListener(new View.OnClickListener(){
                public void onClick(View v) {
                    if (holder.Ibelieve.getCurrentTextColor() == Color.parseColor("#007aff"))
                    {
                        ...
                        Integer newcount = Integer.parseInt(map.get(C_COUNT).toString()) + 1;
                        holder.txtCount.setText(newcount+" Believes");
                    ...
                    else
                    {
                        ...
                        holder.Ibelieve.setTextColor(Color.parseColor("#007aff"));
                        myaskid = map.get(C_ASKID).toString();
                        Integer newcount = Integer.parseInt(map.get(C_COUNT).toString());
                        holder.txtCount.setText(newcount+" Believes");
                        new ibelieveaction().execute();
                    }
                    //new ibelieveaction().execute();
            ...
            holder.spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                @Override
                public void onItemSelected(AdapterView<?> parent, View view, int positioninspinner, long id) {

                    String selectedText = parent.getItemAtPosition(positioninspinner).toString();
                    myposition = position;
                    if (selectedText.equals("Delete")) {
                        myaskid = map.get(C_ASKID).toString();
                        deletecur(myaskid, user_name);
                    } else if (selectedText.equals("Report")) {
                        reportcur(curid, user_name);
                    }
        ...
        public class ibelieveaction extends AsyncTask<String, Void, String> {
            ...
            protected String doInBackground(String... strings) {
                ...
                try {
                    //URL url = new URL(strings[0]);
                    URL url = new URL(C_SERVER_PROD + "ibelieveaction.php");
                    HttpURLConnection urlConnection =
                            (HttpURLConnection) url.openConnection();
                    urlConnection.setRequestMethod("POST");
        ...
        protected void reportcur(final String userid, final String user_name) {
            final CharSequence[] options = {"Spam, Scam or Fake", "Inappropriate Photo or Information", "Harassment, Bullying or Annoying Behavior", "Cancel"};

            AlertDialog.Builder builder = new AlertDialog.Builder(activity);
            builder.setTitle("Report user " + user_name);
            builder.setItems(options, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int item) {
                    if (options[item].equals("Spam, Scam or Fake")) {
                        new reportuser().execute(userid + "|" + "Spam or Scam or Fake");
                    } else if (options[item].equals("Inappropriate Photo or Information")) {
                        new reportuser().execute(userid + "|" + "Inappropriate Photo or Information");
                    } else if (options[item].equals("Harassment, Bullying or Annoying Behavior")) {
                        new reportuser().execute(userid + "|" + "Harassment, Bullying or Annoying Behavior");
            ...
            protected String doInBackground(String... strings) {
                ...
                try {
                    ...
                    if (!params.equals("|")) {
                        ...
                        myuser = tokens.nextToken();
                        myreport = tokens.nextToken();
                    }
                    //URL url = new URL(strings[0]);
                    URL url = new URL(C_SERVER_PROD + "reportuser.php");
                    HttpURLConnection urlConnection =
                            (HttpURLConnection) url.openConnection();
                    urlConnection.setRequestMethod("POST");
                    ...
                    Uri.Builder builder = new Uri.Builder()
                            ...
                            .appendQueryParameter("auth_password", C_SERVER_AUTH_password)
                            .appendQueryParameter("id", myid)
                            .appendQueryParameter("userid", myuser)
                            .appendQueryParameter("report", myreport);
                    String query = builder.build().getEncodedQuery();
                    OutputStream os = urlConnection.getOutputStream();
                    BufferedWriter writer = new BufferedWriter(
                            new OutputStreamWriter(os, "UTF-8"));
        ...
        protected void deletecur(final String userid, final String user_name) {
            final CharSequence[] options = {"Delete this post" , "Cancel"};

            AlertDialog.Builder builder = new AlertDialog.Builder(activity);
            builder.setTitle("Delete Ask");
            builder.setItems(options, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int item) {
                    if (options[item].equals("Delete this post")) {
                        new deletethisuser().execute(userid);
                    } else if (options[item].equals("Cancel")) {
                        dialog.dismiss();
                    }
                }
        ...
        public class deletethisuser extends AsyncTask<String, Void, String> {
            ...
            protected String doInBackground(String... strings) {
                ...
                try {
                    String askid = strings[0];
                    //URL url = new URL(strings[0]);
                    URL url = new URL(C_SERVER_PROD + "deleteuserconn.php");
                    HttpURLConnection urlConnection =
                            (HttpURLConnection) url.openConnection();
                    urlConnection.setRequestMethod("POST");
                    ...
                    Uri.Builder builder = new Uri.Builder()
                            ...
                            .appendQueryParameter("auth_password", C_SERVER_AUTH_password)
                            .appendQueryParameter("id", myid)
                            .appendQueryParameter("askid", askid);
                    String query = builder.build().getEncodedQuery();
                    OutputStream os = urlConnection.getOutputStream();
                    BufferedWriter writer = new BufferedWriter(
                            new OutputStreamWriter(os, "UTF-8"));
